...
async function runAll(req: NextRequest) {
  ...
  const r1 = await fetch(`${base}/refresh-market?week=${week}`, { method: 'POST', cache: 'no-store' });
  const j1 = await r1.json().catch(() => ({}));

  const r2 = await fetch(`${base}/confidence?week=${week}`, { method: 'POST', cache: 'no-store' });
